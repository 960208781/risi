...
public class TB_Move : MonoBehaviour
{
    ...
    void HandleDrag(DragGesture gesture)
    {
        if (!enabled)
            return;
        if (gesture.Fingers.Count > 1) return;

        if (gesture.Phase == ContinuousGesturePhase.Started)
        {
            if (dragObj != null && gesture.Selection != null)
            {
                Debug.LogError(gesture.Selection.name + "    " + dragObj);
                if (gesture.Selection == dragObj)
                {
                    dragging = true;
                }
                else
